...
def CNIH2dependentGluA2Plasticity(ds,dc,vp,alpha,beta_0,beta,eta,gamma,lo):
    ...
    savesimsettings(1,type, time_steps, lo,"{0}/protocol_{1}.json".format(op_dir, date_time),beta_factors=b_factors,
...
def CNIH2IndependentGluA2Plasticity(ds,dc,vp,alpha,beta_0,eta,gamma,lo):
    ...
    e_factor.append(e0)
    time_steps.append(t_step0)
    model_params = [dc, ds, vp, Lamda_pc, Lamda_ps, alpha , beta_0, eta*e0, omega, gamma,
                    Jcin, Jsin, dx]
    t_range = [0, t_step0]
    t_eval = np.arange(0, t_step0, dt)
    ...
    model_params = [dc, ds, vp, Lamda_pc, Lamda_ps, alpha, beta_0, eta * e1, omega, gamma,
                    Jcin, Jsin, dx]
    t_range = [0, t_step1]
    ...
    print("date and time:", date_time)
    saveoutput(op_dir, date_time, data_mat, total_tps, 100, baseline_param_file)
    savesimsettings(2, type, time_steps,"{0}/protocol_{1}.json".format(op_dir, date_time),eta_factors=e_factor)
