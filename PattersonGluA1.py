...
def AMPATimeDynamics(t, y, Dc, Ds, Vp, lamdac, lamdas, alpha, beta, eta, omega, gamma, Jcin, Jsin, dx):
   ...
   """
    ...
    dpcdt[0] = Endocytosis(ps[0], alpha[0]) - Exocytosis(pc[0], beta[0]) - Degrdation(pc[0], lamdac) \
               + 2. * Jcin * (1 / dx - Vp / Dc) + Dc * (- 2. * (1 + dx * Vp / Dc) * pc[0] + 2. * pc[1]) / dx ** 2 - (
                           Vp / dx) * ((1 + 2 * dx * Vp / Dc) * pc[0] - pc[1])
    ...
    dpsdt[0] = Exocytosis(pc[0], beta[0]) - Endocytosis(ps[0], alpha[0]) - Degrdation(ps[0], lamdas) \
               + 2. * Jsin / dx + Ds * (2. * ps[1] - 2. * ps[0]) / dx ** 2 - SpineExchange(ps[0], pspine[0], eta[0], omega,
                                                                                           gamma[0])
    dpsdt[1:-1] = Exocytosis(pc[1:-1], beta[1:-1]) - Endocytosis(ps[1:-1], alpha[1:-1]) - Degrdation(ps[1:-1], lamdas) \
                  + Ds * np.diff(ps, 2) - SpineExchange(ps[1:-1], pspine[1:-1], eta[1:-1], omega, gamma[1:-1])
    dpsdt[-1] = Exocytosis(pc[-1], beta[-1]) - Endocytosis(ps[-1], alpha[-1]) - Degrdation(ps[-1], lamdas) \
                + Ds * (- 2. * ps[-1] + 2. * ps[-2]) / dx ** 2 - SpineExchange(ps[-1], pspine[-1], eta[-1], omega, gamma[-1])
...
def patterson2010GluA1Plasticity(beta,gamma, lo, x_sdx):
    """
    ...
    model_params = [D_c, D_s, V_p, Lamda_pc, Lamda_ps, alpha * np.ones(P_c_init.shape), beta_step0, eta* np.ones(P_c_init.shape), omega, gamma_step0,
                    Jcin, Jsin, dx]
    ...
    model_params = [D_c, D_s, V_p, Lamda_pc, Lamda_ps, alpha * np.ones(P_c_init.shape), beta_step1, eta* np.ones(P_c_init.shape), omega, gamma_step1,
                    Jcin, Jsin, dx]
    ...
    g2 = 1/1.
    new_y_init = data_mat[:, -1]  # Bleach(data_mat[:, -1],lo,x_sdx)
    beta_step2, gamma_step2 = PlasticityExperiment(beta, gamma, lo, x_sdx, f2, g2)
    t_step2 = 9*60  # running for 50 secs
    factors.append(f2)
    g_factors.append(g2)
    time_steps.append(t_step2)
    model_params = [D_c, D_s, V_p, Lamda_pc, Lamda_ps, alpha * np.ones(P_c_init.shape), beta_step2,
    ...
    model_params2 = [D_c, D_s, V_p, Lamda_pc, Lamda_ps, alpha * np.ones(P_c_init.shape), beta_steplast, eta* np.ones(P_c_init.shape), omega,
                     gamma_steplast,
                     Jcin, Jsin, dx]
    ...
    plt.plot((total_tps-t_step0)/60, beta_profile,label=r"$\beta$")
    plt.plot((total_tps-t_step0)/60, gamma_profile,label=r"$\gamma$")
    plt.legend()
    # breakpoint()
    plt.savefig("{0}/fig_protocol_{1}.pdf".format(op_dir, date_time),dpi=300)
    savesimsettings(3, time_steps, factors,g_factors, [loc], x_span, "{0}/protocol_{1}.txt".format(op_dir, date_time))
    plt.show()


def savesimsettings(num_steps, time_steps, factors,g_factors, locations, x_span, protocol_file):
    ...
    with open(protocol_file, 'a') as the_file:
        ...
        the_file.write("parameter changed = beta, gamma")
        the_file.close()
...
patterson2010GluA1Plasticity(beta_array,gamma_arr, location, x_span_dx)
