...
for ti in range(1,t_points):
    ...
    P_s_new =  np.linalg.solve(C, D.dot(P_s_now) + BC(P_c_now,dt,alpha,2.0*K_2*dx*Jsin/D_s))
...
plt.plot(x_grid, P_c_now)
plt.plot(x_grid, P_s_now)
plt.show()
...
plt.xlabel('x'); 
plt.ylabel('t')
heatmap = ax1.pcolor(x_grid, t_grid, P_C, vmin=0., vmax=1.2)
heatmap = ax0.pcolor(x_grid, t_grid, P_S, vmin=0., vmax=1.2)
